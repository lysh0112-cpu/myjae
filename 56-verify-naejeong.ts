...
function main() {
  ...
  head('⑫ 🔴🔴 교재에서 찾기 — 로컬 (AI 안 부름)')
  {
    ...
    ok(/교재에서 못 찾았어요/.test(page),
      '⛔ ★못 찾으면 사실대로 말합니다 [대표님 ㉮]')
    /*  🔴 ★차례 — «상담 목적» 이 «교재에서 찾기» 보다 «위» 입니다 [대표님 2026-09-15]
     *     처음에는 반대였는데 «써 보니 이상하다» 고 하셨습니다.
     *     ⇒ 넓은 갈래를 «먼저» 고르고, 없을 때 «찾는» 것이 자연스럽습니다. */
    {
      const iPurpose = page.indexOf('상담 목적 <span')
      const iFind = page.indexOf('교재에서 찾기 <span')
      const iRun = page.indexOf('onClick={run}')
      ok(iPurpose > 0 && iFind > iPurpose && iRun > iFind,
        '🔴 ★차례가 «상담 목적 → 교재에서 찾기 → 내정 보기» 입니다 [대표님]')
      ok(/위 대분류에서 골라 보세요/.test(page),
        '★못 찾았을 때 «위» 를 가리킵니다 (차례가 바뀌었으므로)')
    }
  ...
  head('⑭ 🔴🔴 본문 용어 — 눌러서 설명 보기')
  {
    ...
    /*  🔴 ★«이름만» 있는 자리도 눌립니다 — 2026-09-15 [대표님]
     *     처음엔 «글 속» 낱말만 붙여서 —
     *     「띠로 보는 오늘 丑 → ★퇴식」 · 「음력 1월 寅 ★금조건」 의 «제목 줄» 이
     *     ★안 눌렸습니다. 대표님이 화면을 보고 짚어 주셨습니다. */
    ok(/const sinButton = /.test(page3), '🔴 ★«이름만» 있는 자리용 단추가 있습니다')
    /*  ⚠️ ★붙는 자리는 «다섯» 입니다 —
    ...
    ok(/isSipsungName\(data\.unsi\.sipsung\)/.test(page3),
