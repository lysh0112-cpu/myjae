...
import { calcCoupleComplement, complementPromptBlock } from '@/lib/saju/coupleComplement'
...
export interface CouplePerson {
  ...
  // ★ "서로 채워주는 용신·귀인" 계산용 (선택). 없으면 그 블록만 빠진다.
  solarMonth?: number
  solarDay?: number
...
// 명리 뼈대(합·충·조후)는 나이로 바뀌지 않지만, 같은 궁합이라도
// 신혼이냐 노년이냐에 따라 마음에 와닿는 현실 맥락이 다르므로 톤만 맞춘다.
// 두 사람 나이가 다르면 더 많은 쪽(관계의 국면을 좌우)을 기준 삼되 양쪽을 함께 고려.
function marriedAgeGuide(p1?: number, p2?: number): string {
  const ages = [p1, p2].filter((a): a is number => typeof a === 'number' && a > 0)
  if (!ages.length) return ''
  const ref = Math.max(...ages)   // 부부 국면은 대개 나이 많은 쪽 생애주기를 따라감
  let ctx: string
  if (ref < 35) {
    ctx = '이제 막 가정을 꾸린 신혼·젊은 부부입니다. 서로 맞춰가는 적응, 자녀 계획, 두 사람만의 미래를 그리는 설렘에 마음이 가 있습니다.'
  } else if (ref < 50) {
    ctx = '가정을 함께 일궈가는 중년 부부입니다. 자녀 양육과 교육, 살림과 재물의 안정, 바쁜 일상 속 서로를 향한 마음을 다시 챙기는 일이 관심사입니다.'
  } else if (ref < 65) {
    ctx = '오랜 세월을 함께한 원숙한 부부입니다. 자녀가 자라 품을 떠나는 시기, 둘만의 시간을 새로 가꾸는 일, 건강과 노후를 나란히 준비하는 마음이 큽니다.'
  } else {
    ctx = '인생의 황혼을 나란히 걷는 노년 부부입니다. 서로가 가장 가까운 벗이자 의지처이며, 지나온 정을 되새기고 남은 길을 다정히 동행하는 데 마음이 있습니다.'
  }
  return `[부부의 지금 — 이 나이대의 마음에 맞춰 풀 것]
- ${ctx}
- 위 생애 맥락에 맞는 현실적 관심사와 다정함으로 풀되, 나이를 직접 말하지는 마세요. 젊은 부부에게 노후를, 노년 부부에게 육아를 논하지 않도록 결을 맞춥니다.`
}
...
export function buildCoupleTongbyeonPrompt(
  ...
  // 부부 궁합일 때만 나이대 톤 지침을 넣는다. 연인은 기존 그대로(빈 문자열).
  const ageBlock = input.mode === 'married'
    ? marriedAgeGuide(input.person1.age, input.person2.age)
    : ''
...
- 이 등급을 자연 비유로 감싸 전하되, 숫자는 언급하지 마세요.`
  ...
  // ★ 서로 채워주는 인연(용신+귀인) — solar 정보가 둘 다 있을 때만 계산.
  //   연재쌤 방향: "당신은 내게 없는 걸 채워주는 사람, 내 귀인"의 근거.
  let complementBlock = ''
  const pA = input.person1, pB = input.person2
  if (pA.solarMonth && pB.solarMonth && pA.dayStem && pB.dayStem) {
    try {
      const comp = calcCoupleComplement(
        { yearPillar: pA.yearPillar, monthPillar: pA.monthPillar, dayPillar: pA.dayPillar, hourPillar: pA.hourPillar, dayStem: pA.dayStem, solar: { month: pA.solarMonth!, day: pA.solarDay ?? 1, hourBranch: pA.hourBranch ?? null } },
        { yearPillar: pB.yearPillar, monthPillar: pB.monthPillar, dayPillar: pB.dayPillar, hourPillar: pB.hourPillar, dayStem: pB.dayStem, solar: { month: pB.solarMonth!, day: pB.solarDay ?? 1, hourBranch: pB.hourBranch ?? null } },
      )
      complementBlock = complementPromptBlock(comp, pA.name || '첫 번째 분', pB.name || '두 번째 분')
    } catch { /* 실패 시 블록 없이 진행 */ }
  }
  ...
  const formatBlock = hasQ
    ? `[답변 형식 — 고른 질문에만 답하기. 고른 질문 수와 카드 수가 정확히 같아야 함]
...
※ 반드시 위에 나열된 ${questions.length}개 질문 카드만 만드세요. "두 사람의 큰 그림" 같은 요약 카드나, 묻지 않은 주제 카드를 절대 추가하지 마세요.
※ 단, 위 "서로 채워주는 인연" 재료가 있으면 — 새 카드를 만들지는 말고 — 질문 답변 중 어울리는 대목이나 마지막 응원에서 "서로에게 없는 걸 채워주는 고마운 사이"라는 점을 한두 문장으로 자연스럽게 곁들이세요.`
    : `[답변 형식 — 전체 총평, 카드로 나눠서]
■ 두 사람의 첫인상 — (한 줄 요약 제목)
   (3~4문단. 두 본바탕이 만났을 때 서로에게 어떤 존재인지, 등급의 느낌. 숫자 없이.)

■ 서로를 살리는 점 — (제목)
   (2~3문단. 좋은 근거를 자연 비유로. ★위 "서로 채워주는 인연" 재료가 있으면
    반드시 이 카드에서 풀어내세요 — 한쪽이 상대에게 없는 기운을 채워주는 점,
    서로가 서로에게 힘이 되는 귀한 인연이라는 점을, 전문용어 없이 다정하게.)
