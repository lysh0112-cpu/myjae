...
/*  ★십성을 가릴 때 쓰는 «본기 천간» 표 (교재 48쪽 도표).
 *  ⚠️ sajuMbti.ts:81 과 ★«같은 값» 입니다 — 화면 위 십성 표가 그것을 씁니다.
 *     ⛔ 한쪽만 고치지 마십시오. 고치면 ★정재·편재가 또 갈립니다 (검사 23 ⑩).
 *  ⚠️ 지장간 표와는 «다른 물건» 입니다. 쓰임이 다르면 잣대도 다릅니다. */
 ...
 *
 *  [겪은 일]  화면 위 십성 표는 «편재 14.3% · 정재 없음» 인데,
 *    아래 「리더십과 재물 운용」 은 ★「정재 5점으로 안정형」 이라 했습니다.
 *    ⇒ 한 화면 안에서 ★정재와 편재가 «뒤바뀌었습니다».
 *
 *  [까닭]  ★지지를 보는 방식이 «두 가지» 였습니다 —
 *    십성 표   지지 → ★본기 천간(BRANCH_BONGI)으로 바꾼 뒤 음양을 봄
 *    여기      지지 → ★글자 그대로 YANG_BRANCH 로 음양을 봄
 *    ⇒ 巳 는 «겉은 음, ★속은 丙(양)» 입니다. 그래서 갈렸습니다.
 *
 *  🔴 ★겉과 속이 다른 글자가 «넷» 입니다 —
 *      巳 겉음·속 丙(양)   亥 겉음·속 壬(양)
 *      子 겉양·속 癸(음)   午 겉양·속 丁(음)
 *    ⇒ 이 넷이 재성일 때 ★정재와 편재가 뒤바뀌었습니다.
 *
 *  [고침]  ★십성 표와 «같은 방식» 으로 맞춥니다 — 지지는 본기 천간으로 바꿔서 봅니다.
 *  ⛔ 되돌리지 마십시오. 한 화면 안에서 ★두 가지 방식을 쓰면 또 갈립니다. */
function jeongPyeon(dayStem: string, ch: string, isStem: boolean): '정' | '편' {
  const dayYang = YANG_STEM.has(dayStem)
  const asStem = isStem ? ch : (BRANCH_BONGI[ch] ?? ch)
  const chYang = YANG_STEM.has(asStem)
