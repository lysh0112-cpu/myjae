...
export function calcHaerak(input: HaerakInput): HaerakResult | null {
  ...
  /*  ⑧ 년 · 월 · 일이 «어디로» 가는가
   *  ──────────────────────────────────────────────────────────────
   *  🔴🔴 ★낱말이 헷갈리는 자리입니다. 값으로 적습니다 —
   *
   *        년(年)  →  ★«위» 에 놓는 괘   (상괘)
   *        월(月)  →  ★«아래» 에 놓는 괘 (하괘)
   *        일(日)  →  동효 (1 · 2 · 3)
   *
   *  ⚠️ ★교재는 위에 놓는 것을 «내괘(內卦)», 아래에 놓는 것을 «외괘(外卦)» 라 부릅니다.
   *     ⇒ 주역의 보통 말(내괘=아래 · 외괘=위)과 ★«반대» 입니다.
   *     ⇒ 그래서 「내괘를 아래로 내려라」 는 말이 나옵니다. ⛔ 그렇게 하면 괘가 깨집니다.
   *
   *  ⛔⛔ ★년과 월을 맞바꾸지 «마십시오».
   *     류 님 노트 스캔(음 66.1.12 · 26년) 으로 값을 재 보았습니다 —
   *        년 77 ÷ 8 = 5 → 巽   ·   월 45 ÷ 6 = 3 → 離
   *        巽 «위» · 離 «아래»  ⇒ 風火家人 = ★가인 53   (스캔과 같음 ✅)
   *        맞바꾸면          ⇒ 火風鼎     = ★정  73   (스캔과 다름 ❌)
   *     ⇒ 노트 일곱 건이 ★6/7 → 0/7 로 무너집니다 (2026-09-14 값으로 확인).
   *     ⇒ 검사 54 ㉒ 가 이 스캔을 «값으로» 지킵니다.
   *  ────────────────────────────────────────────────────────────── */
  ...
  /*  ⑩⑪ ★«아래» 괘에서만 동효를 뒤집고, 위 괘와 이어 부릅니다
   *      (교재는 아래 괘를 «외괘» 라 부릅니다 — 위 주석 참고)
   *  ⛔ ★위 괘를 뒤집지 마십시오. 뒤집으면 노트 일곱 건이 «전부» 달라집니다.
   *  ⇒ 아래 괘만 뒤집으므로 동효는 ★1·2·3 만 나옵니다 (4·5·6 은 영영 안 나옵니다).
   *     류 님 스캔 — 가인(巽 위 · 離 아래)의 아래 離 셋째 줄을 뒤집어 震
   *                ⇒ 巽 위 · 震 아래 = 風雷益 = ★익 61  (스캔과 같음 ✅) */
