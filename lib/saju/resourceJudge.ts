...
// ══════════════════════════════════════════════════════════════════
//  ★★[확정] 2026-08-01 (43부) — 대표님 확정 «현행 배점 유지»
//
//   [무엇을 쟀나]  29-measure-resource-weights.ts 로 일곱 안을 쟀습니다.
//     표본 세 글자 12,000 · 외자 4,000 · 씨앗 고정.
//     ★자기 검산 — 되짚기가 judgeResource 와 16,000/16,000 일치했습니다.
//
//   [드러난 것 셋]
//     ① 🔴 «흐름 무게만» 낮춰서는 자원 별점이 «한 톨도» 안 움직입니다.
//          흐름 30→20→15→10 어느 쪽이든 6칸 · 최다 43.0% 로 같습니다.
//          까닭 — 별점은 칸 «안» 의 비율(flowScore / W_FLOW)로 냅니다.
//                 분자·분모가 함께 줄어 비율이 그대로입니다.
//          ⇒ 「별점 몰림을 풀려고 흐름을 낮춘다」는 «듣지 않습니다». 다른 물음입니다.
//
//     ② ⚠️ 반대로 «자원 축 안의 몫» 을 옮기면 별점이 크게 뭉칩니다.
//          사주상극 몫 1/4(현행) 43.0% → 1/2 69.0% → 3/4 73.8%(칸 5개, 외자 83.5%)
//          까닭 — 사주 상극은 55.9% 가 «0개» 라 만점이 쌓입니다.
//          ⇒ 교재 51쪽에 가까이 갈수록 41부에 71%→31% 로 푼 자리가 되돌아옵니다.
//            «교재에 가까워지는 것» 과 «별점이 퍼지는 것» 이 서로 맞바꾸는 관계입니다.
//
//     ③ ★7/30 이 가혹해진 것은 «흐름을 낮춰서» 가 아니라 «용신을 올려서» 였습니다.
//          흐름10·용신60  아쉬움 37.2%   ← 되돌린 그 안
//          흐름10·용신40  아쉬움 19.6%   ← 용신을 안 올린 판
//          현행           아쉬움 16.5%
//          ⇒ 위 「7/30 되돌림」 주석의 진단이 숫자로 확인되었습니다.
//
//   ★결정 — 현행 배점을 «그대로 둡니다» (2026-08-01 대표님 확정).
//     분포가 가장 완만하고(아쉬움 16.5%) 별점도 가장 퍼져 있습니다(최다 43.0%).
//
//   ⚠️⚠️ 이 값을 고치려는 다음 세션에게 —
//     ① 먼저 npm run measure:resource 로 «지금» 분포를 재십시오.
//     ② 39부 3-1장 ①(가·나·다)과 교재 151쪽 「별점을 매기는 것이 맞는가」는
//        여전히 «열려» 있습니다. ★이 배점과 «같은 결정» 입니다. 따로 답하면 어긋납니다.
//     ③ 위 ①때문에 「흐름 무게」와 「별점 몰림」은 «다른 손잡이» 입니다. 섞지 마십시오.
// ══════════════════════════════════════════════════════════════════
...
export function resourceFactsBlock(v: ResourceVerdict, profile: SajuOhaengProfile): string {
  ...
  if (clashLeft > 0) {
    ...
    // ★2026-07-31 (40부 6차) — 교재 2장 51쪽의 근거를 함께 실습니다 (대표님 지시)
    L.push('')
    L.push(`[${CLASH_VIEW_BOOK.page} — 상극을 보는 눈]`)
    L.push(`· ${CLASH_VIEW_BOOK.notBad}`)
    L.push(`· ${CLASH_VIEW_BOOK.excessWarn}`)
    L.push(`· ${CLASH_VIEW_BOOK.tooltip}`)
    // ★교재가 «직접 쓴» 풀이만 붙입니다. 비유를 지어내지 마십시오 (교훈 EJ)
    for (const k of f.links) {
      if ((k.rel !== '순극' && k.rel !== '역극') || k.exempted) continue
      const g = clashGloss(k.from, k.to)
      if (g) L.push(`· ${g}`)
    }
  }
  return L.join('\n')
...
// ══════════════════════════════════════════════════════════════════
//  ★2026-07-31 (40부 6차) — 상극을 «어떻게 볼 것인가», 교재 2장 51쪽
// ══════════════════════════════════════════════════════════════════
//
//  [교재 원문]  『작명개운법』 2장 51쪽
...
//     화 오행이 많으면 오히려 화의 힘을 빼는 토, 수, 금이 와서
//     화의 힘을 약화시켜 주는 것이 바람직하다.」
//
//  ★두 가지를 말합니다 —
//    ① 상극 자체는 나쁜 것이 아니다
//    ② 과다한 오행에는 «상생이 오히려» 부정적이다      ← 이쪽이 덜 알려져 있습니다
//
//  그리고 교재는 상극을 «누르는 말» 로 적지 않습니다 (51~52쪽 풀이).
//    木剋土  나무는 뿌리로 흙을 잡아 준다
//    土剋水  흙은 둑이 되어 물을 가두어 둔다
//  ★손님 화면에는 이 «교재의 말» 을 쓰십시오. 우리가 비유를 지어내면
//    교훈 EJ(추정 전사)에 걸립니다.
//
//  ⚠️⚠️ 이것은 «해설» 입니다. 감점 구조는 «그대로» 입니다 (대표님 확정).
//     → 말은 더 또렷해지고 점수는 그대로이므로, 교훈 EV(「흠이 아니다」 적고
//       점수 깎지 말 것)의 간격이 «넓어집니다». 39부 3-1장 ① 결정 때
//       이 점을 함께 놓으십시오.

/** 교재 51쪽 — 상극을 보는 관점. ★AI 재료로 그대로 나갑니다 */
export const CLASH_VIEW_BOOK = {
  page: '작명개운법 2장 51쪽',
  /**
   * ① 상극 자체를 흠으로 보지 않는다
   * ⚠️ 교재 원문은 「…나쁘다고 할 수 없다」 입니다. 그 낱말을 «재료에 두지 않았습니다» —
   *    부정문이라도 낱말이 있으면 AI 가 끌어 씁니다 (16-verify ⑦ 금지어 검사).
   *    ★뜻은 그대로 두고 말만 바꿨습니다 (교훈 BR·EG).
   */
  notBad: '상생이 언제나 이로운 것도, 상극이 언제나 흠인 것도 아니라고 봅니다.',
  /** ② 과다한 오행에는 상생이 오히려 부정적이다 */
  excessWarn: '어떤 기운이 이미 과다한데 그것을 더 살려 주면 오히려 부담이 될 수 있고, '
    + '그때는 그 기운을 덜어 주는 쪽이 바람직하다고 봅니다.',
  /** ③ 교재가 직접 쓴 «상극의 긍정» 비유 — ★지어낸 말이 아닙니다 */
  gloss: {
    목극토: '나무는 뿌리로 흙을 잡아 줍니다.',
    토극수: '흙은 둑이 되어 물을 가두어 둡니다.',
    수극화: '물은 불을 다스립니다.',
    화극금: '불은 쇠를 녹여 모양을 냅니다.',
    금극목: '쇠는 나무를 다듬습니다.',
  } as Readonly<Record<string, string>>,
  /** ④ 한 줄로 옮긴 말 — 손님 툴팁·상세 해설용 */
  tooltip: '오행의 상극은 «무조건 억누르는 것» 이 아니라, '
    + '넘치는 기운을 조율하고 가다듬어 주는 «제어» 로도 봅니다.',
} as const

/** 상극 두 오행 → 교재의 풀이 한 줄. 없으면 null */
export function clashGloss(a: Ohaeng | null, b: Ohaeng | null): string | null {
  if (!a || !b) return null
  const key = CONTROLS[a] === b ? `${a}극${b}` : CONTROLS[b] === a ? `${b}극${a}` : null
  return key ? (CLASH_VIEW_BOOK.gloss[key] ?? null) : null
}
