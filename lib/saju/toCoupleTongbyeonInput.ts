...
function personBlock(p: CouplePersonInput, label: string, need?: Set<Need>): string {
  ...
  //     "늦봄" 처럼 단정하지 말고 "봄이 무르익는 무렵" 같은 부드러운 결로 준다.
  const SEASON_POS: Record<string, string> = {
    寅: '봄이 열리는 이른 봄의 결', 卯: '봄이 한창 무르익는 결', 辰: '봄이 무르익어 여름으로 건너가는 결',
    巳: '여름이 열리는 이른 여름의 결', 午: '여름이 한창인 결', 未: '여름이 익어 가을로 건너가는 결',
    申: '가을이 열리는 이른 가을의 결', 酉: '가을이 한창 무르익는 결', 戌: '가을이 익어 겨울로 건너가는 결',
    亥: '겨울이 열리는 이른 겨울의 결', 子: '겨울이 한창 깊어가는 결', 丑: '겨울이 익어 봄으로 건너가는 결',
  }
  if (SEASON_POS[monthBranch]) {
    lines.push(`- 태어난 무렵: ${SEASON_POS[monthBranch]} (이 결의 느낌만 살리고, "늦봄·초겨울" 같은 단정적 시기나 다른 시기를 지어내지 말 것)`)
  }
