...
- "가장 강한 기운/약한 기운"의 큰 그림은 맨 첫 카드(타고난 당신)에서 한 번만 다룹니다. 그 뒤 질문 카드들에서는 이 강약 요약을 다시 처음부터 반복하지 마세요. 각 질문은 그 질문의 명리 연결 부위로 곧장 들어가세요.
...
■ 타고난 당신 — (한 줄 요약 제목)
   (3~4문단. "가장 강한 기운/약한 기운"을 중심으로 이 사람의 큰 그림. 이 통변 전체를 관통하는 캐릭터. 강약 요약은 여기서만.)

${questions.map((q) => `■ ${q.category} — (제목)\n   (3~4문단. 이 카드는 반드시 아래 명리 연결이 가리키는 부위만을 주인공으로 답하세요: "${q.link}". 강한/약한 기운 요약을 다시 반복하지 말고, 이 질문에 딱 맞는 부위로 곧장 들어가 앞 카드와 겹치지 않는 새로운 이야기를 하세요. 질문: ${q.question})`).join('\n')}
