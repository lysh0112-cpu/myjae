...
export interface MulsangTongbyeonInput {
  ...
  hourKo?: string                    // ★태어난 시각 (예: '한낮(午시)') — 그림의 시간대와 맞춰야 한다
}
...
(마지막) 따뜻한 마무리 응원 2~3문장. 이모지 하나로 끝맺기.`
  ...
  return [
    ...
    // ★그림은 이 시각의 하늘로 그려진다. 해설이 다른 시간대를 말하면 그림과 어긋난다.
    input.hourKo
      ? `[그림 속 시간대] ${input.hourKo}\n- 그림의 하늘은 이 시각으로 그려져 있습니다. 밤/낮을 반대로 말하지 마세요.\n- 계절(겨울 등)과 시각(한낮 등)은 다른 것입니다. "겨울이니 어둡다"처럼 섞지 마세요.`
      : '',
    '',
    ilganBlock,
    '',
