...
export const ORDER: Array<{ key: string; title: string; len: string }> = [
  ...
  // ★2026-07-29 — 시험 날짜를 넣으면 AI 가 쓰는 대목. (대표님 지시)
  //   ⚠️ 여기 등재해야 keyOfTitle 이 잡아 카드로 붙습니다.
  //      등재를 빠뜨리면 그 대목이 맺음말 상자로 흘러갑니다. (30부 2장)
  { key: 'dday', title: '시험 당일 실전 준비', len: '4~5문장' },
]
...
[★목표를 반드시 부르세요]
· 손님이 목표를 적어 주셨습니다. **첫 대목과 맺는말에서 그 이름을 직접 부르세요.**
  «목표하시는 곳» 같은 두루뭉술한 말 말고, 적어 주신 그대로 불러 주세요.
...
${plan.map(o => `■ ${o.title} — ${o.len}`).join('\n')}${
  v.examDate ? `
■ 시험 당일 실전 준비 — 4~5문장
   ★반드시 쓰세요. ${v.examDate} 그날의 기운을 짚는 자리입니다.
   · 그날의 일진·월운·세운이 겹쳐 어떤 결이 되는지 먼저 말하고,
   · 그다음 «그날 아침을 어떻게 보내면 좋은가»·«마음을 어디에 두면 흔들리지 않는가» 를 주세요.
   · 컨디션·집중·긴장 다스리기처럼 그날 바로 쓸 수 있는 것으로.
