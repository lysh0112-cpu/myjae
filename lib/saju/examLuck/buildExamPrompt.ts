...
export const ORDER: Array<{ key: string; title: string; len: string }> = [
  ...
  // ★올해 결론 한 문장을 더 붙이라 했으니 자리를 늘렸습니다.
  { key: 'examkind', title: '어떤 시험에 힘이 실리나', len: '4~6문장' },
  ...
  // ★2026-07-29 — 사주 근거를 한 줄 더 붙이라 했으니 자리를 늘렸습니다.
  { key: 'susi', title: '수시와 정시', len: '4~5문장' },
...
/**
 * ★2026-07-29 — 대목마다 «그 자리에서» 읽을 지시를 붙인다. (대표님 지시)
 *
 *   [무엇이 문제였나] 지시를 프롬프트 앞쪽에 블록으로 열세 개나 쌓았습니다.
 *     그런데 «쓸 대목» 목록은 79% 지점에 있고, 그 뒤로 재료가 더 옵니다.
 *     AI 는 마지막에 본 것을 씁니다. 그래서 앞의 지시를 잊고
 *     재료(«丙午년으로 조심입니다»)를 그대로 옮겨 적었습니다.
 *   [어떻게] 지시를 «대목 바로 아래» 로 옮깁니다. 쓰는 그 자리에서 읽게 합니다.
 *     ★멀리 있는 훈계보다 곁에 있는 한 줄이 셉니다.
 */
function cardHint(key: string, v: BuildExamPromptArgs): string {
  const L: string[] = []
  if (key === 'examday' && v.examDate) {
    L.push(`· ${v.examDate} 그날이 어떤 결인지 «한 문장» 으로 먼저.`)
    L.push('· 재료의 «일진 · 천간 ○○ · 지지 ○○» 를 보고, 그 십성이 이 사람에게 어떻게 작용하는지.')
    L.push('· ★그다음이 알맹이입니다 — 그날 아침을 어떻게 보낼지 / 시험 중 집중이 흐트러질 때 무엇을 할지 /')
    L.push('  실수를 막는 법 / 마음이 흔들릴 때 붙잡을 한마디. 네 가지를 구체적으로.')
    L.push('· ★간지만 나열하고 끝내지 마세요. "조심입니다" 로 끝내면 안 됩니다.')
  }
  if (key === 'examkind') {
    L.push('· 십성마다 나열하고 끝내지 마세요. 그건 누구에게나 같은 말입니다.')
    L.push(`· ★마지막에 «그 가운데 ${new Date().getFullYear()}년 ${v.name}님에게는 ○○ 기운이 가장 세게 듭니다» 를 한 문장 붙이세요.`)
  }
  if (key === 'susi') {
    L.push('· «수시가 맞습니다» 로 끝내지 마세요. 원국의 무엇 때문인지 한 줄 더 쓰세요.')
    L.push('· 손님이 이미 전형을 골랐다면 «둘 중 고르라» 고 하지 말고, 고른 길에서 무엇을 살릴지 말하세요.')
  }
  // ★같은 지시를 두 대목에 붙이면 AI 가 같은 말을 두 번 씁니다. 첫 대목에만 붙입니다.
  if (key === 'years') {
    if (v.targetBlock) L.push('· 손님이 적어 준 목표 이름을 여기서 한 번 불러 주세요.')
    if (v.levelTrack) L.push('· ★성적대와 계열을 올해 기운과 «한 문장 안에» 엮으세요. 따로 말하면 뜻이 약해집니다.')
  }
  if (key === 'dayun' && v.conditional) {
    L.push('· 위 [조건] 지시 가운데 이 대목에 맞는 것을 여기서 다루세요.')
  }
...
[★목표를 «이름 그대로» 부르세요]
· 위 [손님이 고른 것] 의 목표 이름을 여는말과 맺는말에서 그대로 부르세요.
...
[판정 재료 — 아래 «쓸 대목»에서 쓸 사실들입니다]
${material}${salBlock}${bookBlock}

════════════════════════════════════════
[쓸 대목 — 이대로 차례차례 쓰세요]
════════════════════════════════════════
${plan.map(o => `■ ${o.title} — ${o.len}\n${cardHint(o.key, v) || '   · 위 재료를 근거로 문장 속에 녹여 쓰세요.'}`).join('\n\n')}

■ 맺는말 — 3~4문장
   · ${v.targetBlock ? '손님이 적어 준 목표 이름을 다시 한 번 불러 주며 맺으세요.' : '따뜻하게 맺으세요.'}
   · 위 [맺음말] 뜻을 담되 그대로 베끼지 말고 앞말과 이어지게 푸세요.

이제 위 차례대로 써 주세요. **각 대목의 · 지시를 반드시 지키세요.**`
}
