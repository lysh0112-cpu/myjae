...
export function judgeJobChangeNatal(saju: Pillar[]): JobChangeHit[] {
  ...
  // ★화(火) 관성 과다 — 교재 191쪽 둘째 뒷부분 "화(火) 관성이 과다한 경우"
  //
  //   [2026-07-27 대표님 확정 — "관성이 화인 경우" 다]
  //     교재의 火 는 말 그대로 관성 오행이 화(火)라는 뜻이다.
  //     나를 극하는 것이 화이려면 일간이 금(金)이어야 한다. 庚·辛 일간만 해당한다.
  //     다른 일간은 관성이 화가 아니므로 아예 안 잡는다.
  //
  //     ⚠️ 전에는 관성 오행을 안 가리고 다 잡았다(관살 과다로 읽었다).
  //        그러면 여덟 중 여섯 일간에게 없는 말이 나간다.
  //
  //   원국의 성질이라 여기서 본다. 해마다 되풀이될 말이 아니다.
  {
    const gwanCount = all.filter(x => x === '정관' || x === '편관').length
    // ★관성이 화(火)일 때만 — 곧 금(金) 일간일 때만
    if (GWAN_EL[n.dayStem] === '화' && gwanCount >= 3) {
      out.push({
        row: JOB_CHANGE.find(r => r.key === 'gwanChung')!,
        why: `일간이 ${n.dayStem}(금)이라 관성이 화(火)인데 ${gwanCount}자로 과다합니다.`,
      })
    }
  }
