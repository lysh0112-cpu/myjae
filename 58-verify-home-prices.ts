...
function main() {
  ...
  head('⑬ 🔴 큐보드·골프온 바로가기 — 토글 [대표님]')
  {
    ...
    /*  🔴 ★관리 화면이 «홈만» 이라고 말하면 안 됩니다 —
     *  ⚠️ 10부에 「홈에서 숨겼어요」 만 보고 ★다 숨긴 줄 알았고,
     *     ★여섯 자리가 남아 있었습니다 (대표님이 화면에서 찾아내심).
     *  ⇒ ★설명 줄 · 묻는 말 · 끝난 뒤 알림 — «셋 다» 자리를 일러 주는지 봅니다. */
    const sisBlk = st.slice(Math.max(0, st.indexOf('sisterLinks') - 1400), st.indexOf('sisterLinks'))
    ok(SUB_MUST.every(w => sisBlk.includes(w)),
      '🔴 ⚠️ ★사이트 설정 «설명 줄» 도 일곱 자리를 일러 줍니다 (홈만이 아닙니다)')
    const askBlk = tg.slice(tg.indexOf("flag === 'sisterLinks'"))
    ok(/일곱 자리/.test(askBlk),
      '🔴 ⚠️ ★«물을 때» 도 일곱 자리가 함께 돌아온다고 일러 줍니다')
    ok(/일곱 자리에서 모두 숨겼어요/.test(tg) && !/^\s*: '✓ 홈에서 숨겼어요',$/m.test(askBlk),
      '🔴 ⛔ ★끝난 뒤 알림이 «홈에서 숨겼어요» 라고만 하지 않습니다')
  ...
  head('⑭ 🔴🔴 토스 결제 — 충전')
  {
    ...
    /*  ══ 🔴🔴 포인트충전 업종 요건 ════════════════════════════════
     *  [토스페이먼츠 회신 2026-09-23] 업종이 ★«포인트충전» 으로 확정됐습니다.
     *  [가이드 4쪽 3번] 「★임의 금액 입력 후 충전하는 결제 방식은 이용이 «불가능».
     *    반드시 ★10만원 이하의 금액을 «선택» 하도록 구현해 주세요」
     *  [회신 2. 조치] · 충전금액 한도를 ★1회 10만원으로 제한
     *                · 홈페이지 «내» 에 ★환불 정책 기재
     *                · 약관에 ★이용·환불기간 «결제시점으로부터 1년» · 양도 불가 · 결제수단 환불
     *  ⛔ 되돌리면 ★심사에서 반려됩니다. */
    {
      const chg = strip(R('app/wallet/charge/page.tsx'))
      //  ⛔ ★손님이 «적어 넣는» 칸이 없어야 합니다
      ok(!/placeholder="직접 입력"/.test(chg) && !/setCustom/.test(chg),
        '🔴🔴 ⛔ ★충전 금액을 «적어 넣는» 칸이 «없습니다» (고르기만 합니다)')
      //  ⛔ ★고를 수 있는 금액이 «모두» 10만원 이하라야 합니다 — 숫자를 못 박지 않고 «셉니다»
      const amts = (R('app/components/common/WalletPanel.tsx')
        .match(/CHARGE_AMOUNTS = \[([^\]]*)\]/) || [])[1] ?? ''
      const nums = amts.split(',').map(x => Number(x.trim())).filter(n => n > 0)
      ok(nums.length > 0 && Math.max(...nums) <= 100000,
        `🔴 ⛔ ★한 번에 «10만원 이하» 만 고를 수 있습니다 (가장 큰 값 ${Math.max(...nums).toLocaleString()}원)`)

      //  🔴 ★손님이 «결제하는 그 화면» 에 환불 정책이 있어야 합니다
      const MUST: [RegExp, string][] = [
        [/결제하신 날로부터 1년/, '이용·환불 기한(결제시점 1년)'],
        [/7일 안에 취소/, '7일 청약철회'],
        [/환불은 결제하신 수단으로/, '결제수단 환불'],
        [/다른 회원에게 넘기실 수 없어요/, '양도 불가'],
      ]
      for (const [re, name] of MUST) {
        ok(re.test(chg), `🔴 ⛔ ★충전 화면에 «${name}» 이 적혀 있습니다 (토스 요건)`)
      }

      //  ⛔ 약관 제7조도 ★같은 말이라야 합니다
      const tx2 = R('app/components/legal/termsText.ts')
      ok(/결제시점으로부터 1년 이내/.test(tx2),
        '🔴 ⛔ ★약관이 «결제시점으로부터 1년» 이라고 적습니다')
      ok(!/최종 이용일로부터 1년/.test(tx2),
        '⛔ ★「최종 이용일」 기준으로 되돌아가지 않았습니다 (기간이 늘어납니다)')
      ok(/결제가 이루어진 수단으로 진행됩니다/.test(tx2) && /양도할 수 없습니다/.test(tx2),
        '🔴 ⛔ ★약관에 «결제수단 환불» 과 «양도 불가» 가 있습니다')
      ok(/1회 충전 금액은 100,000원을 넘을 수 없습니다/.test(tx2),
        '⛔ ★약관에도 «1회 10만원» 한도가 적혀 있습니다')
    }
