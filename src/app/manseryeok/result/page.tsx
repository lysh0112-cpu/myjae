...
const BRANCH_LIST = [
  {char:"子",name:"자",animal:"🐭"},{char:"丑",name:"축",animal:"🐮"},
  {char:"寅",name:"인",animal:"🐯"},{char:"卯",name:"묘",animal:"🐰"},
...
function calcSaju(year:number,month:number,day:number,hourIdx:number|null){
  ...
  const monthStemBase=((year-4)%5)*2;
  ...
  const dayStem=HEAVENLY_STEMS[(jdn+4)%10];
  const dayBranch=EARTHLY_BRANCHES[(jdn+6)%12];
      ...
      <main className="pt-20 pb-36 px-4 space-y-4">
        <div className="rounded-2xl p-4" style={{background:"linear-gradient(135deg,#3C3489 0%,#2a2075 100%)",border:"1px solid rgba(250,199,117,0.2)"}}>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {[`${gender}성`,`${calType} ${year}.${month}.${day}`,hourIdx===null?"시 미지정":`${BRANCH_LIST[hourIdx]?.char}시`].map(item=>(
              <span key={item} className="text-sm font-semibold px-3 py-1 rounded-full" style={{background:"rgba(255,255,255,0.1)",color:"#FAC775"}}>{item}</span>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-2xl p-5" style={{background:"#2C2C2A",border:"1px solid rgba(255,255,255,0.07)"}}>
          ...
          {done&&aiResult&&(
            <div>
              <div className="rounded-xl p-4 mb-4" style={{background:"rgba(60,52,137,0.15)",border:"1px solid rgba(60,52,137,0.3)"}}>
...
export default function ResultPage(){
  return(
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center" style={{background:"#1a1a18"}}><div style={{color:"#FAC775"}}>로딩 중...</div></div>}>
      <ResultContent/>
    </Suspense>
  );
}
