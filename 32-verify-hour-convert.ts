...
console.log('\n━━ ㊵ ★여덟 단계를 프롬프트로 엮기 (프리미엄 궁합 4차) ━━')
{
  ...
  for (const rel of ['부부', '연인'] as const) {
    ...
    //    ⛔ 재료 이름(step8Timeline)과 지시문이 ★«짝» 이어야 합니다. 한쪽만 바꾸면 못 찾습니다.
    check(sp.includes('앞으로의 십년의 운 — 두 분'), `★${rel} — 8단계 재료가 들어갑니다`)

    // ★뼈대에 새 대목 셋이 있는가
    check(/1-2\. 두 분은 어떤 분인가/.test(sp), `★${rel} — 「1-2 두 분은 어떤 분인가」`)
    check(/4-2\. 함께 살아가는 결/.test(sp), `★${rel} — 「4-2 함께 살아가는 결」`)
    check(/6-2\. 앞으로의 십년의 운/.test(sp), `★${rel} — 「6-2 앞으로의 십년의 운」`)
  ...
  //  ⛔ «열 해» 도 남겨 두어야 합니다 — ★이미 저장된 궁합 기록이 그 제목입니다
  check(/t\.includes\('십년의 운'\)/.test(pg) && /t\.includes\('열 해'\)/.test(pg),
    `★「앞으로의 십년의 운」 아이콘이 있고, 옛 제목(열 해)도 남아 있습니다`)
...
console.log('\n━━ ㊷ ★★목업 정본 화면·분량 개편 (44부 22차) ━━')
{
  ...
  check(/앞으로의 십년의 운: ★해마다/.test(sp), `★「앞으로의 십년의 운」 분량 명시`)
...
console.log('\n━━ ㊼ ★사주 MBTI 화면 — 숫자를 걷어내고 «말» 로 (44부 28차) ━━')
{
  ...
  // ★2026-08-03 (44부 34차) — 명리 말을 «괄호 안» 으로 되살렸습니다. (대표님 지시)
  //   🔴 [무엇이 있었나] 28차에 「비겁·식상」을 «전부» 걷어냈더니
  //      ★사주 화면인데 «사주가 안 보였습니다» — 「주체성」이 비겁인지 알 길이 없었습니다.
  //   ⚠️ 이 그물이 그 «틀린 것» 을 지키고 있었습니다. 뜻을 바로잡습니다 —
  //      「쉬운 말이 «먼저» 오는가」가 본디 재려던 것입니다.
  check(/비겁: '주체성\(비겁\)'/.test(eng) && /인성: '사고력\(인성\)'/.test(eng),
    `★쉬운 말이 «먼저», 명리 말은 괄호 안입니다`)
  check(!/비겁: '비겁'|재성: '재성'/.test(eng),
    `⚠️ 명리 말«만» 쓰지 않습니다 — 쉬운 말이 앞에 옵니다`)
...
console.log('\n━━ (55) ★종이 차례 · 성인 직업 거르기 (44부 43차) ━━')
{
  ...
  // 🔴 학생에게 «직장 이야기» 가 나가지 않는가 (44부 45차)
  //   [무엇이 있었나] 「계열과 학과」(성인에게 안 냄)만 빼고,
  //     ★「핵심 직무」·「조직 성향」(학생에게 안 냄)은 «안 뺐습니다».
  //     ⇒ 14세 손님 통변에 ★「이직·조직 성향」이 그대로 나갔습니다.
  //   ⚠️ 카드는 빈 채로 오는데 plan 은 «열쇠» 로만 골라 대목으로 시켰습니다.
  check(/const emptyKeys = new Set\(v\.cards\.filter\(c => c\.lines\.length === 0\)/.test(bp),
    `🔴★lines 가 빈 카드는 «대목으로 시키지» 않습니다`)
  check(/\.filter\(o => !emptyKeys\.has\(o\.key\)\)/.test(bp),
