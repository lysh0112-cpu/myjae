...
console.log('\n━━ ⑲-I ★화살표 · 요약 카드 · 하단 위계 (43부 27·28차) ━━')
{
  ...
  // ③ 하단 — ★2026-08-05 (47부 21차) «두 줄» 로 바뀌었습니다 [대표님 지시]
  //   [무엇이 바뀌었나]  43부 28차의 «세 켜»(①으뜸 ②버금 ③끝)를 거두고
  //     ★버튼 넷을 «두 줄» 로 모았습니다 —
  //       윗줄  🖨️ A4 저장·출력  |  📜 작명 보관함
  //       아랫줄 다른 한자 보기   |  + 새 이름 짓기
  //   ⚠️ 그래서 옛 검사 셋을 «고쳤습니다» —
  //       「A4 명품 작명서 인쇄 · PDF 저장」 문구 → ★「A4 저장·출력」 로 줄임
  //       「끝은 조용한 글줄」            → ★버튼이 되었습니다
  //       「글자 13.5/600」               → ★13/600 (한 줄에 둘이라 줄임)
  //   ⛔ 되돌리시려면 ★화면과 검사를 «함께» 되돌리십시오.
  check(/A4 저장·출력/.test(S.cert), `★A4 버튼이 있습니다`)
  check(/background: p\.disabled \? '#EDE7E0' : '#c8783c'/.test(cert),
    `★A4 는 «채운 색» 입니다 (나머지는 테두리만)`)
  ...
  check(/display: 'flex', gap: 8, marginTop: 4, marginBottom: 10/.test(nr),
    `★아랫줄 둘을 «나란히» 두었습니다`)
  check(/\+ 새 이름 짓기/.test(S.nr) && /📜 작명 보관함/.test(S.nr),
    `★두 줄에 버튼 넷이 다 있습니다`)
  check(/minHeight: 46/.test(nr), `★넷이 «같은 높이»(46px) 입니다`)
  // ⚠️ 같은 버튼이 두 번 뜨지 않는가
...
console.log('\n━━ ⑲-G 🔴 성씨는 «가문 선택» 이지 «사주 추천» 이 아니다 (43부 23차) ━━')
{
  ...
  // ★2026-08-01 (43부 24차) — 표 «자체» 를 잽니다
  {
    ...
    for (const h of ['망절', '비전'])
      check(rows.some(r => r.h === h), `복성 「${h}」이(가) 있습니다`)
    // ★2026-08-09 대표님 지시 — 어금(魚金)은 걷었습니다. 되살아나지 않았는가
    check(!rows.some(r => r.h === '어금'), `★어금(魚金)은 걷어낸 채입니다`)
  }
...
console.log('\n━━ ⑲-z 🔴 회차 문구 · 보관함 «온전한» 저장 (43부 8차) ━━')
{
  ...
  //  🔴 ★2026-09-09 — «새 모양» 으로 다시 조였습니다 [대표님 「플로우가 이상하다」]
  //     전  이름이 한 자면 「남은 N회」를 «안 말하는가» (갈래로 막았는가)
  //     후  ★이용권을 통째로 걷어, 아예 «언제나» 회차를 말하지 않습니다.
  //     ⛔ 느슨하게 풀지 말고 새 모양으로 조입니다 (57·58부 방식).
  check(/: <>✨ 이 이름 자세히 풀이 보기<\/>/.test(codeOf(nr)) && !/남은 \{remaining\}회/.test(codeOf(nr)),
...
console.log('\n━━ ⑳-b ★하단 정돈 — 띠 배너 · A4 버튼 · 두 칸 ━━')
{
  ...
  // ★2026-08-05 (47부 21차) — 13.5 → ★13 으로 한 번 더 낮췄습니다 [대표님 지시]
  //   까닭: A4 버튼이 «한 줄에 둘» 이 되어 폭이 절반(약 200px)입니다.
  //         13.5 로는 「A4 저장·출력」이 좁은 화면에서 넘칩니다.
  //   ⛔ 다시 올리지 마십시오. 바꾸실 때 ★320px 화면에서 재십시오.
  check(/fontSize: 13, fontWeight: 600/.test(cert),
    `★글자를 낮췄습니다 (15/700 → 13.5 → 13/600)`)
  check(/minHeight: 46/.test(cert), `★A4 도 짝과 «같은 높이»(46px) 입니다`)
...
console.log('\n━━ ㉑-d ★성씨 표 차례 — 통계청 인구수 순인가 (대표님 확정) ━━')
{
  ...
  // ★2026-08-09 대표님 지시로 어금(魚金)을 걷어 126/186 → ★125/185 가 되었습니다.
  //   ⚠️ 43부 확정값(126/186)은 surnameHanja.ts 머리말에 «기록으로» 그대로 둡니다.
  //      숫자가 안 맞는다고 어금을 «되살리지» 마십시오. 대표님이 빼신 것입니다.
  check(keys.length === 125, `★125 소리 그대로입니다 (${keys.length})`)
  check(new Set(flat).size === 185, `★고유 한자 185 그대로입니다 (${new Set(flat).size})`)
...
// ══════════════════════════════════════════════════════════════════
//  🔴🔴 ★2026-09-09 — 보관함에서 열 때 «그 사람» 사주인가  [대표님 지시]
//    「아이 사주를 넣었는데 보관함에 보니 ★남의 사주명식을 불러오고 있어」
//
//   [무엇이 있었나]  보관함 주소에는 ?recordId= 만 실립니다. 사람 정보가 «없습니다».
//     ⇒ 화면이 «본인 profiles» 로 떨어져, 아기 기록을 열어도
//       ★로그인한 분의 사주가 「내 사주」 칸에 떴습니다.
//     ⚠️ 기록에는 person 이 «제대로» 담겨 있었습니다 — 화면이 안 읽었을 뿐입니다.
//
//   ⚠️⚠️ 이것은 ★개인정보가 섞이는 자리입니다 (48부 「앞 손님 풀이가 섞입니다」와 같은 결).
//      ⇒ 사람 눈으로는 «틀린 줄 모릅니다». 화면이 «멀쩡히» 뜨니까요.
//      ⇒ 그래서 곧바로 그물을 칩니다.
// ══════════════════════════════════════════════════════════════════
console.log('\n━━ ㉒-a 🔴 보관함에서 열 때 «그 사람» 사주인가 (2026-09-09) ━━')
{
  const diag = codeOf(read('app/manseryeok/naming/diagnosis/page.tsx'))
  check(/if \(rec\.person\) \{/.test(diag),
    `★저장해 둔 person 을 «봅니다»`)
  check(/setInfo\(\{ \.\.\.rec\.person \}\)/.test(diag),
    `★그 사람 사주로 화면을 채웁니다`)
  check(/if \(sp\.get\('recordId'\)\) return/.test(diag),
    `★본인 프로필이 «덮어쓰지» 않습니다 (보관함이면 빠져나갑니다)`)
  // ⚠️ person 이 «없는» 옛 기록은 예전처럼 본인 사주로 내려가야 합니다 — 막으면 안 됩니다
  check(/const profInfo = await fetchProfileInfo\(\)/.test(diag),
    `⚠️ person 이 없는 옛 기록은 예전처럼 내려갑니다 (막지 않았습니다)`)
}

// ══════════════════════════════════════════════════════════════════
//  ⛔ ★2026-09-09 — 「六 사주 명리적성」을 어디에 보일지  [대표님 지시]
//    ① 「내 아기 명품이름작명에 ★결이 안 맞는데 보여주고 있어」
//    ② 「내이름정밀 분석도 … ★이것은 불필요해.
//        ★우리 서비스가 이미 사주 서비스가 명리적성편이 있거든」
//
//   ⇒ 이름 풀이는 «이름» 이야기로 끝냅니다. 적성은 ★진로적성 서비스가 맡습니다.
//   ⛔ 부품을 «지우지» 마십시오 — 끄기만 했습니다. 되살리려면 값만 바꾸면 됩니다.
// ══════════════════════════════════════════════════════════════════
console.log('\n━━ ㉒-b ⛔ 「六 사주 명리적성」이 이름 풀이에 «안 나오는가» (2026-09-09) ━━')
{
  const diag = codeOf(read('app/manseryeok/naming/diagnosis/page.tsx'))
  const nres = codeOf(read('app/manseryeok/naming/rename/newresult/page.tsx'))
  check(/showAptitude=\{false\}/.test(diag),
    `★내 이름 정밀분석 — 六 을 «안 보입니다»`)
  check(/showAptitude=\{namingKind !== '신생아'\}/.test(nres),
    `★아기(신생아) 작명 — 六 을 «안 보입니다»`)
  //  ⚠️ 부품은 «남아» 있어야 합니다 — 지우면 되살릴 길이 없어집니다
  const apt = read('app/manseryeok/naming/diagnosis/components/NamingAptitude.tsx')
  check(/六\. 사주 명리적성/.test(apt),
...
// ══════════════════════════════════════════════════════════════════
//  🔴 ★2026-09-09 — 「아직 안 돌려준 돈」 이 보이는가  [대표님 지시]
//    「관리자 화면에서 ★되돌려주지 않은 금액들만 따로 모아 볼 수 있으면 좋겠다」
//   ⚠️ 가리는 법 — mc_ledger 에서 ★같은 ref 줄들의 합 (use 크기 − refund 크기).
//   ⛔ [돌려주기] 단추를 넣지 마십시오 — 지갑 함수는 «자기 것» 만 만집니다.
// ══════════════════════════════════════════════════════════════════
console.log('\n━━ ㉒-f 🔴 「아직 안 돌려준 돈」 (2026-09-09) ━━')
{
  const ch = read('app/admin/components/CancelledHistory.tsx')
  check(/아직 안 돌려준 돈/.test(ch), `★취소 내역 맨 위에 «모아» 보입니다`)
  check(/from\('mc_ledger'\)/.test(ch) && /\.in\('ref', ids\)/.test(ch),
    `★지갑 내역을 ref(상담 건 id)로 «한 번에» 가져옵니다`)
  check(/kind === 'use'/.test(ch) && /kind === 'refund'/.test(ch),
    `★쓴 줄과 되돌린 줄을 «갈라» 셉니다`)
  check(/안 돌려준 것만/.test(ch) && /전체/.test(ch), `거르개 둘이 있습니다`)
  check(/회원 지갑에서 손으로 넣어 주십시오/.test(ch),
    `⚠️ 손으로 넣으시라고 «알려» 줍니다 (관리자는 남의 지갑을 못 되돌립니다)`)
  //  ⛔ 눌러도 안 되는 단추를 만들지 않았는지
...
// ══════════════════════════════════════════════════════════════════
//  🔴 ★2026-09-09 — 「내 아이 명품작명」 과 「한자 바꾸기」 는 ★값이 다릅니다
//    [대표님]  「내 아이 명품작명은 ai결제하기가 있나?」
//
//   ⚠️⚠️ 두 서비스가 ★«같은 화면»(newname) 을 씁니다 — kind 로만 갈립니다.
//      아기(신생아)  → ★naming_baby_ai    그 밖(개명) → ★naming_hanja
...
console.log('\n━━ ㉒-j 🔴 작명 — 아기와 개명의 «값» 이 갈리는가 (2026-09-09) ━━')
{
  ...
  //  🔴 ★2026-09-09 — 결과 화면의 이용권 UI 도 걷었습니다 [대표님 「플로우가 이상하다」]
  //     「이용 가능 횟수를 모두 사용했어요 / ★다시 결제하고 이어하기」 가 떴습니다.
  //     ⇒ 값은 「이 이름으로 확정하기」 에서 «한 번» 받고, 상세 풀이가 그 안에 듭니다.
  //     ⚠️ 상세 풀이는 한 번 부르면 저장되어 다시 볼 때 «안 부릅니다» (loadDetail).
  //     ⛔ 「남은 N회」·「다시 결제하고 이어하기」 를 되살리지 마십시오.
  //  ⚠️⚠️ ★codeOf 로 «주석을 걸러» 봅니다 — 2부 1-6 「검사가 주석의 낱말을 글자로 잡습니다」.
  //     제가 «왜 걷었는지» 를 주석에 적어 두어, read() 로 보면 그 글자에 걸립니다.
  const nres = codeOf(read('app/manseryeok/naming/rename/newresult/page.tsx'))
  check(!/다시 결제하고 이어하기/.test(nres),
    `⛔ 「다시 결제하고 이어하기」 가 «없습니다»`)
  check(!/이용 가능 횟수를 모두/.test(nres),
    `⛔ 「이용 가능 횟수를 모두 사용했어요」 가 «없습니다»`)
  check(!/남은 \{remaining\}회/.test(nres),
    `⛔ 「남은 N회」 가 «없습니다» (43부 8차 — 회차를 말하지 않습니다)`)
  check(/localStorage\.removeItem\(NAMING_PASS_KEY\)/.test(nres),
    `★결과 화면도 남은 이용권을 «지웁니다»`)
}
...
console.log('\n━━ ㉒-l 🔴 약관·방침이 «워드 원본과 짝» 인가 (2026-09-10) ━━')
{
  //  🔴 [왜 이 그물이 필요한가]
  //     약관 글이 ★«두 벌» 입니다 — 워드(대표님 보관용)와 코드(손님이 보는 것).
  //     2026-09-10 에 ★실제로 어긋났습니다 —
  //       · 코드에 「제4조 약관의 개정」이 «통째로 빠져» 있었습니다
  //       · 제5조 탈퇴 문장이 워드와 «다른 말» 이었습니다
  //     ⇒ 사람 눈으로는 못 잡습니다. 뼈대만이라도 검사가 지킵니다.
  //     ⛔ 워드를 고치시면 ★termsText.ts · privacyText.ts 도 «함께» 고치십시오.
  const t = codeOf(read('app/components/legal/termsText.ts'))
  const v = codeOf(read('app/components/legal/privacyText.ts'))

  //  ① 조 번호가 1부터 «빠짐없이» 이어지는가 (빠지거나 겹치면 잡습니다)
  const nums = (s: string) =>
    [...s.matchAll(/title: '제(\d+)조/g)].map(m => Number(m[1]))
  const seq = (a: number[]) => a.length > 0 && a.every((n, i) => n === i + 1)
  check(seq(nums(t)), `★약관 조 번호가 1..${nums(t).length} 로 «빠짐없이» 이어집니다`)
  check(seq(nums(v)), `★방침 조 번호가 1..${nums(v).length} 로 «빠짐없이» 이어집니다`)

  //  ② 워드와 «조 수» 가 같은가 — 조항이 통째로 빠지는 것을 잡습니다
  check(nums(t).length === 11, `★약관이 «11개 조» 입니다 (워드와 같음)`)
  check(nums(v).length === 11, `★방침이 «11개 조» 입니다 (워드와 같음)`)

  //  ③ 빠지면 «법이나 심사» 가 걸리는 조항들이 살아 있는가
  check(/제4조 \(약관의 개정\)/.test(t), `⛔ 「약관의 개정」이 있습니다 — 고칠 근거입니다`)
  check(/제8조 \(청약철회와 환불\)/.test(t), `⛔ 「청약철회와 환불」이 있습니다 — 전자상거래법`)
  check(/제10조 \(개인정보 보호책임자\)/.test(v), `⛔ 「보호책임자」가 있습니다 — 없으면 법 위반`)
  check(/제6조 \(국외 이전\)/.test(v), `⛔ 「국외 이전」이 있습니다 — 고지 대상`)

  //  ④ 세 앱을 «함께» 덮는다고 적혀 있는가 (큐보드·골프온이 이 한 벌을 씁니다)
  check(/cue\.myjae\.kr/.test(t) && /golf\.myjae\.kr/.test(t), `★약관이 세 앱을 «함께» 덮습니다`)
  check(/cue\.myjae\.kr/.test(v) && /golf\.myjae\.kr/.test(v), `★방침이 세 앱을 «함께» 덮습니다`)

  //  ⑤ 회사 정보를 ★다시 적지 않고 «가져다» 쓰는가 (두 벌이 되면 어긋납니다)
  check(/COMPANY/.test(t) && /COMPANY/.test(v), `⛔ 회사 정보를 companyInfo 에서 «가져다» 씁니다`)
  check(!/296-86-04182/.test(t) && !/296-86-04182/.test(v),
    `⛔ 사업자번호를 약관 글에 «다시 적지» 않았습니다`)

  //  ⑥ ★가짜 값이 «다시» 들어오면 잡습니다 (큐보드에서 실제로 나온 자국입니다)
  const fake = /000-00-00000|070-0000-0000|201호/
...
console.log('\n━━ ㉓-b 🔴 연재쌤 규칙 보충 검사(43번)가 «전체 검사» 에 묶여 있는가 (2026-09-11 · 6부) ━━')
{
  ...
  check(/tsx 48-verify-ai-abort\.ts/.test(pkg), `★npm run verify 가 48번 검사(화면이 끊으면 AI 호출도 끊기)를 돕니다`)
  check(/tsx 49-verify-exam-engine\.ts/.test(pkg), `★npm run verify 가 49번 검사(합격운 · 취업운 엔진 계산)를 돕니다`)
}
