...
function main() {
  ...
  head('④ 🔴 특화 분석 = 전체 서비스 «같은 모양» [대표님]')
  {
    ...
    /*  🔴🔴 [대표님 2026-09-22 「여기 테두리들을 어떻게 처리했지?」 · 가장자리를 잘라 보내심]
     *  ⛔ ★backdrop-filter 가 들어오면 두 가지가 «동시에» 망가집니다 —
     *     ① 카드 «뒤» 의 페이지 바탕(베이지)까지 끌어와 ★흰 바탕이 «뿌연 크림» 이 됩니다
     *     ② ★«새 바탕 층» 을 만들어 부모의 inset 띠를 «덮어» ★금빛 띠가 안 보입니다
     *  ⚠️ 바탕이 «반투명 그라데이션» 이던 시절의 값이었습니다.
     *  ⇒ ★아래 카드에는 «원래부터» 없습니다. «있는 쪽» 이 다른 것이라 양쪽을 다 봅니다. */
    ok(!/backdropFilter/.test(best) && !/backdrop-filter/.test(best),
      '🔴🔴 ⛔ ★특화 카드에 backdrop-filter 가 «없습니다» (띠를 덮고 바탕을 흐립니다)')
    ok(!/backdropFilter/.test(solo),
      '⛔ ★전체 서비스 카드에도 «없습니다» (양쪽이 같습니다)')
