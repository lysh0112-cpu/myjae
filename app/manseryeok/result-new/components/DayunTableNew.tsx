...
export default function DayunTableNew({solarYear,solarMonth,solarDay,gender,monthGanji,yearStem,dayStem,currentYear,birthYear,ilgan,yeonjji,iljji,hourIdx=null}:Props) {
  ...
  return (
    <div style={{background:'#fff',border:'0.5px solid #9c7a58',borderRadius:16,overflow:'hidden',marginBottom:10,fontFamily:"'Apple SD Gothic Neo','Noto Sans KR',sans-serif"}}>
      {/* 헤더 */}
      <div style={{display:'flex',alignItems:'center',gap:8,padding:'12px 16px 10px',borderBottom:'0.5px solid #b99a7d'}}>
        ...
        <span style={{fontSize:11,padding:'2px 8px',borderRadius:10,background:'#fff3e9',border:'0.5px solid #9c7a58',color:'#c8783c',fontWeight:600}}>현재 {age}세</span>
        ...
        <div onClick={()=>setSel(null)} style={{position:'fixed' as const,inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',padding:20,zIndex:900}}>
          <div onClick={e=>e.stopPropagation()} style={{maxWidth:240,width:'100%',background:'#fff',borderRadius:16,padding:'16px 14px',position:'relative' as const}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:3,flexWrap:'wrap' as const}}>
              ...
              <span style={{fontSize:10,fontWeight:600,color:'#c8783c',background:'#fff3e9',border:'0.5px solid #9c7a58',borderRadius:7,padding:'2px 6px'}}>{selD.age}~{selD.age+9}세</span>
            </div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              {[
                ...
                <div key={i} onClick={()=>SAJU_TERMS[item.val]&&setTerm(item.val)}
                  style={{display:'flex',alignItems:'center',justifyContent:'space-between',background:'#faf3ee',border:'0.5px solid #9c7a58',borderRadius:9,padding:'8px 11px',cursor:SAJU_TERMS[item.val]?'pointer':'default'}}>
