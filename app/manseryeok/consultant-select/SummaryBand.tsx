// app/manseryeok/consultant-select/SummaryBand.tsx
type Props = {
  mode: string
  score: string
  names: string
...
export default function SummaryBand({ mode, score, names }: Props) {
  return (
    <div className="flex items-center gap-2 mx-5 mt-4 bg-[#13132a] rounded-xl px-4 py-3 border border-[#252545]">
      <span className="w-2 h-2 rounded-full bg-[#9977ff]" />
      <span className="w-2 h-2 rounded-full bg-[#5599ff]" />
      ...
      {score && (
        <span className="text-[13px] text-[#b8a9ff] font-medium">궁합 {score}점</span>
      )}
    </div>
  )
}
