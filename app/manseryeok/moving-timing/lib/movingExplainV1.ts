...
//
//   [무엇을 말하는가]
//   ① 피한 것   — 공망·충·형을 어떻게 비켜갔는지 (근거를 글자로 짚는다)
...
export function buildDayExplain(a: BuildArgs): DayExplain {
  ...
  if (gmList.length > 0) {
    ...
    avoided.push({
      /*  ★2026-09-13 (7부) [대표님 전체 점검] — 제목을 ★「공망을 비켜갔어요」 로 바꿨습니다.
       *    ① 제목만 «빈자리» 이고 본문은 «공망» 이라 ★두 말을 썼습니다.
       *    ② ★«비키다» 가 맞습니다 — «피해서» 지나가는 것이기 때문입니다.
       *       (비스듬히 «스쳐» 지나가는 쪽 낱말과 헷갈리기 쉽습니다)
       *  ⚠️ 결혼택일도 「공망(기운이 비는 자리)」 이라 씁니다 — ★한 집안입니다. */
      head: '공망을 비켜갔어요',
  ...
  if (people.length > 0) {
    ...
    avoided.push({
      head: '부딪히지 않아요',
      body: `${txt}. 이날 지지는 ${branch}라 비켜갑니다.`,
    })
