      ...
      {isListening && (
        <div className="px-4 pt-2 pb-1">
          <div className="bg-[#13132a] rounded-xl px-3 py-2 flex items-center gap-2">
            ...
            <span className="text-[#5555aa] text-xs flex-1 truncate">
              {interimText || '말씀해주세요...'}
            </span>
            <button onClick={handleVoice} className="text-xs text-[#5555aa] hover:text-white shrink-0">
              완료
            </button>
          </div>
        </div>
      ...
      <div className="flex items-end gap-2 px-3 py-3">
        ...
        <div className="flex-1 bg-[#13132a] border border-[#252545] rounded-2xl px-4 py-[10px] flex items-end gap-2">
          ...
          <button onClick={() => setShowEmoji(prev => !prev)}
            className={`shrink-0 text-[20px] transition-opacity ${showEmoji ? 'opacity-100' : 'opacity-50'}`}>
            😊
          </button>
        </div>
