// app/manseryeok/consultant/components/YongsinProBoard.tsx
'use client'
...
export default function YongsinProBoard({ saju, dayStem, hourIdx }: Props) {
  if (!dayStem || saju.length === 0) return null

  const { track1, track2, isConflict, conflictAdvice, score } =
    calcYongsinPro(saju, dayStem, hourIdx)
    ...
    <div className="rounded-2xl overflow-hidden"
      ...
      <div className="mx-4 mb-3 rounded-xl p-4"
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          {track2.yongsin && (
            <span className="text-xs px-2 py-1 rounded-lg font-bold"
              style={{background:`rgba(${track2.yongsin==='화'?'244,67,54':track2.yongsin==='목'?'76,175,80':track2.yongsin==='수'?'33,150,243':track2.yongsin==='금'?'158,158,158':'255,152,0'},0.2)`,
      ...
      {isConflict && (
        <div className="mx-4 mb-4 rounded-xl p-4"
          ...
          <p className="text-xs leading-relaxed" style={{color:'#b0aec8'}}>
            {conflictAdvice}
          </p>
        </div>
      )}
