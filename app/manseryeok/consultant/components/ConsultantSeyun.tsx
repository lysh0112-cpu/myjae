...
import { SeyunItem } from '@/lib/saju/dayun'
...
export default function ConsultantSeyun({ seyunList, ilgan, yeonjji, iljji }: Props) {
  ...
  return (
    <div className="rounded-2xl p-4" style={{background:'#2C2C2A',border:'1px solid rgba(255,255,255,0.07)'}}>
      <div className="flex items-center gap-2 mb-3">
        <h2 className="text-base font-bold text-white">세운</h2>
        ...
        {displayList.map((seyun, i) => {
          ...
          return (
            <button key={i}
              ...
              <div className="text-[10px] mb-0.5" style={{color: isCurrent ? '#FAC775' : '#8a88a0'}}>{seyun.year}</div>
              <div className="text-base font-bold leading-tight" style={{color: ganColor}}>{seyun.cheongan}</div>
              <div className="text-base font-bold leading-tight" style={{color: jiColor}}>{seyun.jiji}</div>
              <div className="text-[10px] mt-0.5" style={{color:'#8a88a0'}}>{seyun.ganYukchin}</div>
            </button>
          )
        })}
      </div>
        ...
        <div className="mt-3 rounded-xl p-4" style={{background:'rgba(60,52,137,0.2)',border:'1px solid rgba(60,52,137,0.4)'}}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <span className="text-lg font-bold" style={{color: GAN_COLOR[selectedSeyun.cheongan] ?? '#FAC775'}}>{selectedSeyun.cheongan}</span>
              <span className="text-lg font-bold" style={{color: JI_COLOR[selectedSeyun.jiji] ?? '#e0dce8'}}>{selectedSeyun.jiji}</span>
