...
async function getParentSaju(p: PersonInput | null): Promise<{ dayStem?: string; yongsin?: string }> {
  ...
  try {
    ...
    //   /api/lunar 가 돌려주는 양력 월·일을 쓴다. 시를 모르면 null.
    const hb = hour.branch === '?' ? null : hour.branch
    const ys = calcYongsinCompat(sajuPillars, day.stem, data.solarMonth, data.solarDay, hb)
...
function BirthResultInner() {
    ...
    async function run() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          handleSave(result.recommendations, notes, sv, p1, p2)
        }
      } catch {
  ...
  async function handleSave(
    ...
    p1Override?: PersonInput | null,
    p2Override?: PersonInput | null,
  ) {
