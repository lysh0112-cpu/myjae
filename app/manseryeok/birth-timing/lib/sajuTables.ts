...
// ── 오행 매핑 ──────────────────────────────────────────────────────────
import { JIJANGAN as SHARED_JIJANGAN } from '@/lib/saju/yongsinNew'
 ...
 * 지장간 (정기·중기·여기 전부).
 *
 * ★★2026-08-05 — 공용 표(lib/saju/yongsinNew.ts)로 «통일했습니다». [대표님 지시]
 *
 *   [전]  이 파일이 «자기 표» 를 따로 갖고 있었습니다.
 *         저장소에 지장간 표가 ★세 벌이었습니다
 *           lib/saju/yongsinNew.ts              사주보기·궁합·격국·작명
 *           app/…/birth-timing/lib/sajuTables.ts  ← 여기
 *           app/…/result-new/ExpertDetail.tsx     만세력 화면
 *
 *   [무엇이 달랐나 — 전수 대조]
 *     · 정기(본기)는 ★열둘 «전부» 같았습니다.
 *     · 子卯午酉 넷 — 여기를 «간추린» 것뿐. 壬→癸·甲→乙·丙→丁·庚→辛 이라
 *       ★오행으로 환산하면 결과가 «같습니다». 뜻 차이가 아닙니다.
 *     · ★亥 하나만 «뜻이 달랐습니다» — 戊(土)를 넣느냐 마느냐.
 *
 *   [★지워진 판단 — 잊지 마십시오]
 *     ★연재쌤 확정 2026-07-23
 *       「亥에는 戊(土)를 넣지 않는다. ★亥는 토 일간의 뿌리가 될 수 없다(약하다)」
 *     ⇒ 2026-08-05 대표님께서 «통일하자» 하셔서 이 판단을 ★거두었습니다.
 *       ⚠️ 연재쌤이 다시 「亥에 戊는 아니다」 하시면 ★이 파일에서 亥만
 *          되돌리면 됩니다. 위 문장이 그 근거입니다. 지우지 마십시오.
 *
 *   [★결과가 바뀌는가 — 실측했습니다]
 *     후보 ★25,920개(년주9 × 월주12 × 일주60 × 시진4)를 전·후로 맞댔습니다.
 *     ⇒ ★판정이 달라진 후보 «0개». 일곱 항목 전부 0건.
 *     [까닭]  출산택일 시진은 巳午未申 넷뿐인데
 *             巳=戊 · 午=己 · 未=己 · 申=戊 로 ★넷 모두 토를 갖습니다.
 *             그래서 모든 후보가 시지만으로 이미 «토 뿌리» 를 갖습니다.
 *     ⚠️⚠️ 「지금 안 바뀐다」와 「앞으로도 안 바뀐다」는 다릅니다.
 *        ★시진 제약(巳午未申)이 결과를 «가리고» 있는 것입니다.
 *        자연분만을 다루거나 시진을 넓히면 ★戊·己 일간 × 亥 에서 즉시 갈립니다.
 *        (일간12 × 지지12 = 144 짝 중 어긋나는 것은 戊×亥 · 己×亥 «둘»)
 *
 * ⚠️ 이 파일의 차례는 ★«정기먼저» [정, 중, 여] 입니다. 공용 표는 «여기먼저» 라
 *    가져오며 뒤집습니다. 지금 쓰는 두 곳(JIJANGAN_EL · rootSeatsOf)은 순서를
 *    안 보지만, 나중에 순서를 쓰게 될 때를 위해 차례를 지켜 둡니다.
 */
export const JIJANGAN: Record<string, string[]> = Object.fromEntries(
  Object.entries(SHARED_JIJANGAN).map(([b, list]) => [b, [...list].reverse()]),
)
