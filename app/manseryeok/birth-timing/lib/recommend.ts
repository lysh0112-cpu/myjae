...
// 부모 일간과 아기 일간의 관계 가점 (보조, 최대 +10)
function parentBonus(babyDayStem: string, parents: ParentLite[]): { bonus: number; note: string } {
  let bonus = 0
  let note = ''
  ...
  for (const p of parents) {
    ...
    if (isHap) { bonus += 5; note = '부모와 천간합 — 인연이 깊어요'; continue }
...
export async function runBirthTiming(dueDate: string, opts: RunOptions = {}): Promise<BirthResult> {
  ...
  const scored = candidates.map(c => {
    ...
    const pb = parentBonus(c.day.stem, parents)
