...
function NewResultInner() {
  ...
  /**
   * 🔴★2026-08-01 (43부 32차) — «소리와 한자가 어긋난» 이름 (앞 화면은 고쳤습니다)
   *
   *  [무엇이 있었나]  한자를 고르는 화면에서 칸을 옮기는 «동안» 옛 목록이 남아 있어,
   *    「민」 자리에 潤(윤)이 박히는 일이 있었습니다 (辛潤江 → 「신민강」).
   *
   *  ⚠️⚠️ [여기서 잡지 «못하는» 까닭]
   *    저장된 글자에는 «한자와 한글» 만 있고 «그 한자의 본래 소리» 가 없습니다.
   *    潤 이 「윤」인지 알려면 hanja 표에 물어야 하는데, 결과 화면이 그 표를
   *    다시 훑으면 화면이 느려지고 판정과도 갈릴 수 있습니다.
   *    ★그래서 «막는 것은 앞 화면에서» 합니다 (newhanja 의 두 겹).
   *
   *  🔴 PENDING — 이미 저장된 옛 기록에 어긋난 이름이 남아 있을 수 있습니다.
   *     보관함 목록을 한 번 훑어 «소리가 맞는지» 재는 일이 필요합니다.
   *     ⚠️ 여기서 «고치지 마십시오» — 어느 쪽이 손님 뜻인지 알 수 없습니다.
   *        알려 드리고 손님이 다시 고르시게 해야 합니다.
   */
  ...
  return (
    <main style={{ minHeight: '100vh', background: BG, maxWidth: 480, margin: '0 auto', padding: '8px 16px 32px' }}>
      ...
      {/* ★2026-08-05 (47부 9차) — 상담 카드를 «화면 맨 끝» 으로. [대표님 지시]
          ⚠️ 「다른 이름 또 지어보기」 보다 «아래» 입니다.
          ⛔ 다시 위로 올리지 마십시오. */}
      {/* ══════════════════════════════════════════════════════════════
          ★2026-08-05 (47부 7차) — 아기 작명과 어른 개명을 «갈랐습니다». [대표님 지시]
            「아기 작명과 어른 개명을 분리시켜」 / 「어른 개명 = 내 이름 정밀분석」

          [까닭]  이 화면 «하나» 가 둘을 다 그립니다. namingKind 로만 갈립니다.
            그래서 전에는 둘 다 priceKey="naming" 이라
            ★값도 노출토글도 «따로» 둘 수 없었습니다.

          [지금]  namingKind === '신생아'  → ★naming_baby  「내 아기 명품이름」
                  그 밖(개명)             → ★naming       「내 이름 정밀분석」
            ⇒ 관리자 가격 관리에서 «따로» 값을 매기고 «따로» 켜고 끌 수 있습니다.

          ⚠️ consult_prices 에 ★naming_baby 줄이 «있어야» 아기 쪽 버튼이 보입니다.
             줄이 없으면 통째로 숨습니다 (망가지지 않습니다).
