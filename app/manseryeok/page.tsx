  ...
  return (
    <div className="flex items-center gap-3 mb-4">
      <div
        ...
        style={{ background: "linear-gradient(135deg, #3C3489, #4e46b0)" }}
      >
        {icon}
      </div>
      <div>
...
function ResultPreview({ ready }: { ready: boolean }) {
  ...
  return (
    <div className="mt-4 rounded-xl p-4 relative overflow-hidden"
      ...
      <p className="text-[10px] text-center mt-2" style={{ color: "#8a88a0" }}>
        시주는 태어난 시간 입력 후 확정됩니다
      </p>
    </div>
  );
}
...
export default function ManseryeokPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#1a1a18", maxWidth: "430px", margin: "0 auto" }}>
      {/* 상단 헤더 */}
      <header
        className="fixed top-0 z-50 flex items-center justify-between px-4 py-4"
        style={{
      ...
      >
        ...
        <div className="w-9 h-9 rounded-xl flex items-center justify-center"
          style={{ background: "rgba(60,52,137,0.3)" }}>
      ...
      <main className="pt-20 pb-36 px-4 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {(["남", "여"] as const).map((g) => (
                  <button key={g} onClick={() => setGender(g)}
                    ...
                    <span className="text-xl">{g === "남" ? "♂" : "♀"}</span>
                    <span>{g}</span>
                  </button>
                ))}
              </div>
                ...
                {(["양력", "음력"] as const).map((c) => (
                  <button key={c} onClick={() => setCalType(c)}
                    ...
                    style={calType === c
                      ? { background: "#3C3489", color: "#FAC775", border: "1px solid rgba(250,199,117,0.3)" }
                      : { background: "rgba(255,255,255,0.04)", color: "#8a88a0", border: "1px solid rgba(255,255,255,0.08)" }}>
                    <span className="text-xl">{c === "양력" ? "☀️" : "🌙"}</span>
                    <span>{c}</span>
                  </button>
                ))}
              </div>
        ...
        <div className="rounded-2xl p-5"
          ...
          <BirthDateInput year={year} month={month} day={day}
          ...
          <div className={`grid grid-cols-4 gap-2 transition-opacity ${unknownHour ? "opacity-30 pointer-events-none" : "opacity-100"}`}>
            {EARTHLY_BRANCHES.map((b, i) => {
              ...
              return (
                <button
                  key={b.char}
                  onClick={() => { setSelectedHour(i); setUnknownHour(false); }}
                  ...
                  <span className="text-base font-bold leading-none"
                    style={{ color: isSelected ? "#FAC775" : "#e0dce8" }}>
                    {b.char}
                  </span>
                  <span className="text-[10px] mt-0.5" style={{ color: isSelected ? "#FAC775" : "#8a88a0" }}>
                    {b.name}
                  </span>
                  <span className="text-[9px] mt-0.5" style={{ color: isSelected ? "rgba(250,199,117,0.7)" : "#6a6880" }}>
                    {b.time}
                  </span>
                </button>
              );
          ...
          {(selectedHour !== null || unknownHour) && (
            <div className="mt-3 rounded-xl px-4 py-2.5 flex items-center gap-2"
              ...
              <span className="text-sm text-white">
                {unknownHour
                  ? "시주 미지정 (추정 분석)"
                  : `${EARTHLY_BRANCHES[selectedHour!].char}시 (${EARTHLY_BRANCHES[selectedHour!].time}시) 선택됨`}
              </span>
            </div>
          )}
        </div>
                ...
                <button
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold" style={{ color: on ? "#FAC775" : "#c0bcd8" }}>
                          {opt.label}
                        </span>
                        {opt.required && (
      ...
      {/* 고정 하단 버튼 */}
      <div
        className="fixed bottom-0 z-50 px-4 py-4"
        style={{
