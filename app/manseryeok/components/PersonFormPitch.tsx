...
export default function PersonFormPitch({
  ...
  // 시: '모름'이면 unknownHour, 아니면 인덱스 → 라벨
  const initHour = initial?.input.hour
  const [unknownHour, setUnknownHour] = useState(initHour === '모름' || initHour === '' || initHour == null)
  ...
  return (
    <div style={{ background: C.cardBg, borderRadius: 20, overflow: 'hidden', fontFamily: 'inherit' }}>
      ...
      <div style={{ padding: '18px 18px 6px' }}>
        ...
        {/* 생년월일 (직접 입력, signup 방식) */}
        <div style={label}>생년월일 {calType === '음력' && <span style={{ color: C.subLight }}>(음력)</span>}</div>
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 14 }}>
          <input value={year} onChange={e => setYear(onlyNum(e.target.value, 4))} inputMode="numeric" placeholder="1990" style={{ ...numInput, flex: 1.5 }} />
          <span style={{ fontSize: 12, color: C.sub }}>년</span>
          <input value={month} onChange={e => setMonth(onlyNum(e.target.value, 2))} inputMode="numeric" placeholder="1" style={{ ...numInput, flex: 1 }} />
          <span style={{ fontSize: 12, color: C.sub }}>월</span>
          <input value={day} onChange={e => setDay(onlyNum(e.target.value, 2))} inputMode="numeric" placeholder="15" style={{ ...numInput, flex: 1 }} />
