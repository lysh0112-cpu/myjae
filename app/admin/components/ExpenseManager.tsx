...
export default function ExpenseManager() {
  ...
  const save = async () => {
    ...
    if (!amount || Number(amount) <= 0) { alert('금액을 입력해주세요.'); return }
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '8px 4px' }}>
      ...
      <p style={{ color: '#888', fontSize: 13, marginBottom: 20 }}>
        지출을 항목별로 기재합니다. 매출 집계·손익 요약·엑셀 출력은 다음 단계에서 추가됩니다.
      </p>

      {/* 입력 폼 */}
      <div style={{
        border: '1px solid #e5e5e5', borderRadius: 12, padding: 16, marginBottom: 24,
        background: '#fafafa'
      }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
          ...
          <label style={labelStyle}>
            ...
            <input type="text" value={description} onChange={e => setDescription(e.target.value)}
              placeholder="예: Vercel Pro 6월" style={inputStyle} />
          </label>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <div style={summaryBox}>
          <div style={{ fontSize: 12, color: '#888' }}>건수</div>
            ...
            {loading ? (
              ...
              list.map(e => (
                <tr key={e.id} style={{ borderTop: '1px solid #eee' }}>
                  ...
                  <td style={tdStyle}>
                    {e.is_asset
                      ? <span style={{ color: '#555', fontSize: 12 }}>자산</span>
                      : <span style={{ color: '#dc2626', fontSize: 12 }}>비용</span>}
                  </td>
...
const labelStyle: React.CSSProperties = {
  display: 'flex', flexDirection: 'column', gap: 4, fontSize: 12, color: '#555', fontWeight: 500,
}
const inputStyle: React.CSSProperties = {
  padding: '8px 10px', borderRadius: 6, border: '1px solid #ddd', fontSize: 14, background: '#fff',
}
const summaryBox: React.CSSProperties = {
  flex: 1, minWidth: 140, border: '1px solid #e5e5e5', borderRadius: 10, padding: '12px 16px', background: '#fff',
}
