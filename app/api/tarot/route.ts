...
function safeParse(raw: string, body: Body): Interp {
  ...
  try {
    ...
    return {
      ...
      //  🔴 ★2026-09-23 (11부) — 전문가 상담 «운영 종료» [대표님]. ⛔ 되살리지 마십시오.
      advice: '바로 아래 "새로운 질문하기"로 다시 뽑아보셔도 좋아요.',
    }
  }
}
...
export async function POST(req: Request) {
  ...
  const bump = await aiSpeedBump(g.userId, 'tarot')
...
주의: cards 배열은 뽑힌 카드 ${body.cards.length}장을 순서대로 모두 포함하세요.`
     ...
     *  ⚠️ 이 갈래로 들어오는 길은 ★없어졌습니다 (손님 화면에서 신청할 수 없습니다).
     *     그래도 ★글이 남아 있어 그물에 걸렸습니다 — ⛔ «쓰이지 않는다» 고 두지 않습니다.
     *  ⛔ 갈래 자체는 «지우지» 않았습니다. 되살리실 때 말만 되돌리면 됩니다. */
    if (body.mode === 'consultant') {
      interpretation = {
        title: '카드를 모두 뽑았습니다',
        cards: body.cards.map(c => ({
          position: c.position, name: c.name, direction: c.direction,
          meaning: '뽑으신 카드를 그대로 담아 두었습니다.',
        })),
        summary: '뽑으신 카드가 저장되었습니다.',
        advice: '보관함에서 다시 보실 수 있어요.',
      }
    }
